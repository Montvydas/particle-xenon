...
void pollBleUart(void)
{
    while (bleuart.available())
    {
        wakeupUSB();

        // get rid of all the chars if usb hid not read to not later send a bunch of chars
        if (!usb_hid.ready())
        {
            while (bleuart.available())
                bleuart.read();
...
void pollUsbUart(void)
{
    while (Serial.available())
    {
        // get rid of all the chars if not connected to not later send a bunch of chars
...
void connect_callback(uint16_t conn_handle)
{
    ...
    // Do not print anything because will cause chaos
...
void disconnect_callback(uint16_t conn_handle, uint8_t reason)
{
    ...
    // printing here is fine because it will get discarded anyway
    Serial.println();
